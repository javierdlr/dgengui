...
DBUG("    '%s'\n",dat->Name);
				...
				slot[0] = dat->Name[IUtility->Strlen(dat->Name)-1]; // get last char from name
DBUG("    slot #%s (0x%lx)\n",slot,slot[0]+0x30);
				// CNA_UserData holds savestatus index (as number/integer)
				node = IChooser->AllocChooserNode(CNA_CopyText,TRUE, CNA_Text,slot, CNA_UserData,slot[0]+0x30, TAG_DONE);
				if(node) { IExec->AddTail(dgg->savestates_list, node); }
			}
		}
		//if(IDOS->IoErr() == ERROR_NO_MORE_ENTRIES) {}
DBUG("  SortList() savestates\n",NULL);
		IUtility->SortList(dgg->savestates_list, &HK);
	}
...
DBUG("PREVIEW: '%s'\n",filename);
	if(updateButtonImage(filename, NULL, OID_PREVIEW_BTN, dgg->win) == FALSE) // .PNG (previews drawer)
	{
		...
		{
			updateButtonImage(PREVIEWS"/_not_available.png", GetString(&li,MSG_ERROR_GUI_NOPREVIEW), OID_PREVIEW_BTN, dgg->win); // fallback img/txt
		}
	}
...
//DBUG("  dgg->wbs = 0x%08lx\n",dgg->wbs);
		if(dgg->wbs)
		{
			...
			++dgg->myTT.last_rom_run; // listbrowser starts from 0
