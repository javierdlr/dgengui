...
DBUG("  ExamineObjectTags(): '%s' %s%lld bytes\n",dat->Name,"",dat->FileSize);
	...
	if(res_val != 0) {
		...
		if(res_val != 0) {
			...
			// CNA_UserData holds savestatus index (as number/integer)
...
DBUG("  load savestate %ld (0x%08lx)\n",res_val,res_val);
			IUtility->SNPrintf(cmdline, CMDLINE_LENGTH, "%s -s %ld",cmdline,res_val);//savestate_str[0]);
		}
	}
...
// BUTTONS
	...
	if( OBJ(OID_MAIN) ) {
		...
		if(dgg->myTT.last_rom_run<0  ||  dgg->myTT.last_rom_run>res_totnode) { dgg->myTT.last_rom_run = 0; }
DBUG("last_rom_run=%ld (of %ld)\n",dgg->myTT.last_rom_run+1,res_totnode);
...
DBUG("updateList()\n",NULL);
	...
	// Add NO string (MSG_GUI_SAVESTATES_NO) at top/head of the list..
		IExec->AddHead(dgg->savestates_list, node);
		//..and re-attach chooser list
		IIntuition->RefreshSetGadgetAttrs(GAD(OID_SAVESTATES), dgg->win, NULL,
		                                  CHOOSER_Labels,dgg->savestates_list, GA_Disabled,FALSE, TAG_DONE);
	}
